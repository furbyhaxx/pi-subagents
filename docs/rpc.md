...
Session artifacts default to `join(getAgentDir(), 'sessions')/<project>/<root-session-id>/tasks/`, or `<PI_CODING_AGENT_SESSION_DIR>/subagents/<project>/<root-session-id>/tasks/` when that session-root override is set, with sibling `worktrees/`. `getAgentDir()` respects `PI_CODING_AGENT_DIR` (default `~/.pi/agent`). `sessionArtifactDirectory` overrides the base absolutely or relative to the origin project, ahead of the session-root default. The root is recorded and reused on session resume; setting changes affect new sessions only. Storage is persistent, and without an explicit `sessionArtifactDirectory` it follows the session-root override while staying independent of an agent's `session_dir`; unwritable storage is an error, not a silent `/tmp` fallback. No automatic artifact expiration, old-temp migration/deletion, handle reconstruction or interrupted-agent restart is added.
