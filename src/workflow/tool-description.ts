 ...
 *   - `isolation` retains every worktree and reports its path on settle.
...
Pass the script inline via \`script\` — do not Write it to a file first. Every inline invocation automatically persists its script to the session's durable artifact tasks directory (sessionArtifactDirectory, default <agent dir>/sessions/<project>/<session>/tasks) and returns the path in the tool result. To iterate on a workflow, edit that file with Write/Edit and re-invoke SubagentWorkflow with \`{scriptPath: "<path>"}\` instead of resending the full script. A script you will run more than once belongs in \`.pi/workflows/<name>.js\` (or \`.agents/workflows/\`, or \`<agent dir>/workflows/\` for one that follows the user everywhere); call it with \`name: "<name>"\` instead of re-sending the source.
...
Script body hooks:
- agent(prompt: string, opts?: {label?: string, phase?: string, schema?: object, model?: string, effort?: string, isolation?: 'worktree', branch?: string, agentType?: string, gate?: string, resume?: string}): Promise<any> — spawn a subagent. Without schema, returns its final text as a string. With schema (a JSON Schema), the subagent is given a StructuredOutput tool built from it and agent() returns the validated object — no parsing needed. A payload that does not match is rejected back to the child, which corrects it; a child that never answers through the tool gets one more prompt and then fails, so the call returns null — filter after every schema stage. Returns null if the user skips the agent mid-run or the subagent dies on a terminal API error after retries (filter with .filter(Boolean)). opts.label overrides the display label. opts.phase explicitly assigns this agent to a progress group (use this inside pipeline()/parallel() stages to avoid races on the global phase() state — same phase string → same group box). opts.model replaces the agent's configured model fallback list. Omit it by default. For an agent with configured models, set it only to recover or resume that agent when its configured selection is unavailable, not merely to change preference. opts.effort overrides the reasoning effort for this agent call ('minimal' | 'low' | 'medium' | 'high' | 'xhigh' | 'max') — omit to inherit the agent definition's own level, then the parent's; use 'low' for cheap mechanical stages and higher tiers only for the hardest verify/judge stages. opts.isolation: 'worktree' without branch creates a fresh detached linked worktree retained after every outcome — EXPENSIVE (setup time + disk per agent), use when parallel edits would conflict. Completion reports its path. No automatic commit, branch creation, merge, reset, stash, clean or removal occurs; the orchestrating agent must explicitly review it and either integrate its changes then remove it, or discard and remove it. opts.branch: 'feat/x' instead creates or reuses a reusable named linked worktree on that exact local branch, implying worktree isolation. A missing local branch starts at caller HEAD; no fetch or remote guessing. Neither creation mode copies the caller's uncommitted files; reuse exposes the named worktree's existing changes. Named workspaces are never automatically committed, merged, reset, stashed, cleaned or removed. Cannot combine branch with resume or isolation: 'off'; fails if worktrees are disabled, vetoed by agent frontmatter, or the branch is busy. Pass branch as an option, use repository-relative task paths, state allowed edits/validation/commit policy, and never ask parallel agents to write the same branch. Fresh calls reuse files, not conversation. opts.gate: '<command>' runs a shell command after the agent finishes in its effective working directory, before workspace lease release, and requires it to pass — a non-zero exit marks the agent failed and the command's output becomes the error; prefer gate: 'npm test' over asking another agent whether the code looks right. opts.resume: '<label>' continues the child that ran under that label instead of starting fresh. It may be combined with model to recover on another model; it cannot be combined with agentType, effort, isolation, branch, gate or schema. A resumed child keeps its recorded workspace but does not inherit or run a gate; re-verification needs a fresh gated call in the same workspace. Missing or changed branch scope is an error, not a fallback to the parent checkout. opts.agentType uses a custom subagent type instead of the default workflow subagent — resolved from the same registry as the Agent tool; composes with schema. Available types:
{{typeList}}
