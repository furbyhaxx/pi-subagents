...
- **Live widget UI** — persistent above-editor widget with animated spinners, live tool activity, token counts, and colored status icons. Restored completed history stays in `/agents`; only runs interrupted by the prior Pi exit briefly return to this live-status surface. Configurable via `/agents → Settings → Widget`: `all` (every agent), `background` (default — hides foreground runs, which already render inline as the `Agent` tool result), or `off`
...
- **Git worktree isolation** — retained detached copies for one-off isolation, or `branch: "feat/x"` for a retained, reusable linked worktree; neither mode automatically commits, branches, merges, resets, stashes, cleans or removes anything
...
Open an agent from `/agents → Running agents`, FleetView, or the workflow inspector's `c` action. **Steps** is the default: the task stays pinned above grouped activity, and local timestamps include seconds. `t` reads the task independently, Right expands a step inline, a second Right enters its preview, `o` opens retained details, and `Tab` switches to Raw. JSON error payloads are formatted across lines when expanded. Navigation pauses live following; `End` in the history resumes it. See the [viewer guide](docs/conversation-viewer.md) for navigation, child inspection, and controls.
...
By default, foreground and background agents each stream their full conversation to a per-subagent transcript — a JSON-lines file under `<agent dir>/sessions/<project>/<session>/tasks/` by default, or under `<PI_CODING_AGENT_SESSION_DIR>/subagents/<project>/<session>/tasks/` when that session-root override is set (owner-only directories, persistent across restart; see [storage settings](#persistent-settings)). Set `output_transcript: false` on a custom agent to write no transcript path or file for it, or set `outputTranscript: false` in `subagents.json` to make transcripts opt-in for the whole project (frontmatter overrides the project default). This governs **only** the transcript: it is independent of `persist_session` (the pi session on disk), and it does not affect worktree storage (anonymous and named worktrees retain their files and index without automatic Git mutations) or `memory:` (durable files) — set those accordingly if the goal is to keep a run off disk entirely. Background agent completion notifications render as styled boxes:
...
| `isolation` | — | Set to `worktree` for a retained detached copy, or `off` to veto worktrees (frontmatter is authoritative). A caller supplying `branch` against an `off` veto gets an error, not an unisolated run. `none`, `no`, and `false` are accepted spellings of `off` |
...
| `persist_session` | `subagents.json` `rememberAgents` (default `true`) | Persist this subagent as a normal pi session instead of keeping the session in memory only; overrides the `rememberAgents` project default in both directions. It records its spawning session as parent, so it nests under it in `/resume`; with `PI_CODING_AGENT_SESSION_DIR` set the file goes to `<that dir>/subagents`, which pi's own session list does not descend into, and the child is restored into `/agents` instead. The subagent's `.output` transcript is still written either way unless `output_transcript: false` |
...
| `session_dir` | pi default | Optional session directory when `persist_session: true`; omitted uses `<PI_CODING_AGENT_SESSION_DIR>/subagents` when that override is set, otherwise pi's normal session location, and relative paths resolve from the agent cwd. A session outside the parent's session directory (this override, or `isolation: worktree`) is listed separately, so it shows as a root instead of nesting |
...
| `isolation` | `"off"` \| `"worktree"` | no | Without `branch`, `worktree` creates a retained detached copy; `off` does not. Hidden with `branch` when `worktreeIsolation: false` |
...
Workflow `agent()` accepts `branch: "feat/x"` for sequential work in a retained workspace. Gates run in the child's effective cwd before worktree settlement and lease release. For anonymous worktrees, background jobs are quiesced before the gate; if quiescence cannot be confirmed, the gate does not run. Named worktrees skip implicit job control. Resume does not inherit a gate, and explicit `resume + gate` remains invalid; re-verification needs a fresh gated call. Workspace metadata stays separate from schema-validated output; `agent()` still returns text or its validated object. Completion XML includes deduplicated authoritative `{ worktree, cwd? }` records in `<workspaces>`, outside result truncation. Branch calls end journal replay at that position because their files are mutable external state. See the [branch workflow guide](docs/workflows.md#retained-branch-workspaces).
...
**Model recovery** (`maxRetries`, default `3`; `maxModelWraparounds`, default `0`): retries are Pi's own retry classifier and backoff, applied independently to every candidate visit. `maxRetries: 2` means one initial request plus two retries. Wraparounds count additional complete passes: A/B/C with one wrap visits `A → B → C → A → B → C`. Both settings are non-negative integers, snapshot when an invocation starts, and do not change provider-level transport retries. Automatic model switching depends on Pi's private retry coordinator. Required methods are checked at runtime, not gated by Pi's package version; incompatible APIs fail normally rather than silently disabling fallback.
...
**Remember agents** (`rememberAgents`, default `true`): whether subagents persist their pi session, which is what lets [`@handle`](#agent-mentions) reopen an agent's conversation after its in-memory record has been evicted. Two visible consequences of the default: top-level subagents write a session file, and they nest under the session that spawned them in pi's `/resume`; when that parent session is resumed after a Pi restart, its child sessions are restored into `/agents` so their transcripts can be opened. With `PI_CODING_AGENT_SESSION_DIR` set, child session files live in `<that dir>/subagents` — a container pi's own `/resume` listing does not descend into — and the `/agents` restore discovers them from there. Completed historical runs do not enter live widget or FleetView surfaces; a child with no terminal completion record was interrupted by the prior exit and briefly appears in the above-editor widget as aborted or errored. Agents spawned by another agent are excluded — they get no handle, so nothing could reopen their transcript. A custom agent's `persist_session` frontmatter overrides this per agent, in both directions. Toggle via `/agents → Settings → Remember agents`; with it off, handles expire with their record (roughly ten minutes past completion) and `@explore` then starts a fresh agent rather than resuming — the behaviour before this setting existed.
...
**Session artifact directory** (`sessionArtifactDirectory`, omitted by default): base directory for transcripts, inline workflow scripts, journals and default worktrees. Default `join(getAgentDir(), "sessions")`, or `<PI_CODING_AGENT_SESSION_DIR>/subagents` when that session-root override is set; `getAgentDir()` respects `PI_CODING_AGENT_DIR`, otherwise `~/.pi/agent`. An explicit `sessionArtifactDirectory` always wins. Custom paths may be absolute or relative to the origin project; relative values stay relative in settings. `/agents → Settings → Session artifact directory` offers Default and Custom path. A collision-resistant project key and root-session id separate concurrent sessions:

```text
<default container>/<project>/<root-session-id>/
├── tasks/       # .output transcripts, .workflow.js scripts, .workflow.jsonl journals
└── worktrees/   # default worktree container
```

The resolved artifact root is recorded for each root session and reused on resume. Changes apply to **new sessions only**; no live-file moves, automatic migration, expiration or cleanup on shutdown/record eviction. Old temporary files are neither migrated nor deleted. Unwritable storage is reported, never silently replaced with `/tmp`; extension-owned directories remain owner-only. This storage survives ordinary restart/reboot, unless you explicitly choose temporary or externally managed storage. Persistence does **not** restart interrupted agents, reconstruct old agent/workflow handles, or enable cross-session workflow replay. Artifacts are independent of an agent's `session_dir` frontmatter: an explicit `sessionArtifactDirectory` decides, and otherwise the default follows `PI_CODING_AGENT_SESSION_DIR` into the same `subagents/` container as the child's own session file.
...
**Worktree isolation** (`worktreeIsolation`, default `true`): enables retained detached isolation and retained branch workspaces. Off, the `Agent` tool's `isolation` and `branch` fields and their descriptions disappear together on the next pi session; runtime enforcement applies immediately, including workflow, nested, scheduled and RPC paths. Existing anonymous requests keep their downgrade-to-normal-run behavior without a note. Explicit `branch` requests instead **fail** when disabled or vetoed; they never silently run in the parent checkout. The agent-file generator also stops offering `isolation:`. See [Turning worktrees off](#turning-worktrees-off).
...
`options.branch` has the same retained-workspace contract as the `Agent` tool: exact local name, implied isolation, no automatic Git mutation or removal, busy-branch refusal. The extension supplies the root-session artifact and origin anchors; RPC callers cannot retarget those internal fields. Resolved workspace metadata is separate from result prose; a queued spawn may only know the requested branch, not its eventual path.

`options.cwd` (absolute path to an existing directory — anything else returns an error envelope; `null` means unset) runs the agent in a different working directory than the parent session. Its tools operate there and the prompt's environment block describes it, but **`.pi` config still loads from the parent session's project** — the target directory's `.pi` extensions never execute, and its agents/skills/settings are not picked up. Combined with `isolation: "worktree"`, the worktree is created *from* the target directory's repo, the agent works at the equivalent subdirectory inside the detached copy (a monorepo-package cwd stays scoped to that package), and the copy remains registered after settlement and shutdown. With `branch`, the same subdirectory mapping applies to the named linked worktree. In both modes the completion message names the retained path and whether it has changes. Missing or stale registrations fail with an actionable error rather than being forced or silently recreated. Agents with `memory:` keep reading/writing the parent project's memory.
...
Set `isolation: worktree` without `branch` to run an agent in a retained detached git worktree:

```
Agent({ subagent_type: "refactor", prompt: "...", isolation: "worktree" })
```

The agent gets a full, isolated copy of the repository at detached HEAD. The extension retains that worktree after every settlement path: success, turn-limit wrap-up, abort, stop, failure, cancellation and shutdown. It remains detached, whether clean or changed. The extension does not create a preservation branch or automatically commit, merge, reset, stash, clean, remove or prune it.

Completion reports the retained worktree path and its change state. The orchestrating agent must inspect the retained files, choose integration or discard explicitly, and then remove the worktree and prune stale registrations. If integration needs a branch or commit, create it deliberately inside the retained worktree before merging or cherry-picking.
...
If the worktree cannot be created (not a git repo, no commits, or `git worktree add` fails), the `Agent` call fails with a clear error instead of running unisolated — `isolation: "worktree"` is a strict guarantee, not a hint. Failures before `git worktree add` create no workspace. Failures after add during verification still fail the call, but retain and report the acquired path conservatively. The call is reported as a failed tool call, not as a subagent that ran and returned that message, so the model doesn't retry it as if the agent had merely reported a problem. Initialize git and commit at least once, or omit `isolation`.

When a background-jobs runtime answers on the event bus, an anonymous worktree first stops that worktree's background jobs — before a workflow gate — and reports what was stopped (`Stopped 2 background job(s) still running in the worktree: …`). If termination cannot be confirmed — the runtime errored, timed out, or speaks a protocol this build does not know — the failure is reported in the result (or error) and the gate does not run. The worktree is retained in every case; quiescence protects later human review and cleanup rather than authorizing automatic deletion. With no runtime loaded and no job-capable record the step is a no-op; once a runtime was possible for that agent, an unavailable RPC is reported as a failure. Shutdown coordination removes the load-order question: pi-background-jobs asks this extension to run that same settlement over `subagents:rpc:prepare-shutdown` before it disposes its own RPC responder. Named (`branch`) worktrees skip implicit job control and keep their jobs.
...
Named worktrees are **retained** on success, turn limit, abort, stop, failure, cancellation and shutdown. Like anonymous worktrees, the extension never automatically commits, creates a preservation branch, merges, resets, stashes, cleans, removes or prunes them. A fresh call on the same branch reuses files, not conversation; `resume` continues the original conversation and recorded scope and cannot be combined with `branch`. Resume reacquires the repository/branch lease and validates path, repository and checked-out branch; it refuses a missing or changed scope rather than falling back to the parent directory.
...
Results carry authoritative workspace metadata separately from truncated prose: actual branch, worktree path, effective cwd, created/reused and retained status. Queued calls report requested branch and workspace pending, never an invented path. Every worktree completion reports the retained path and change state; named completions say edits remain in the worktree, not to merge a generated branch. Workflow schema output stays separate from workspace metadata. See [storage settings](#persistent-settings) for placement.
...
src/
  ...
  background-jobs-rpc.ts # Outbound stop-worktree call during anonymous-worktree settlement
  ...
  session-dir.ts      # Child session container below PI_CODING_AGENT_SESSION_DIR
  worktree.ts         # Git worktree isolation (acquire, verify state, release leases)
